    ...
    if st.checkbox('데이터프레임 생성'):
        ...
        if selected == radio_menu[0]:
            ...
            if selected_data == data_topic[0]:
                ...
                df_4chart = st.dataframe(df.iloc[:,[0,1,2,5,6,7,8,9,10]].sort_values(['년도','나이대'],axis=0))
            elif selected_data == data_topic[4]:
                df_4chart = st.dataframe(df.iloc[:,[0,1,2,11,12]].sort_values(['년도','나이대'],axis=0))
            elif selected_data == data_topic[5]:  # 설문형 결과
                df_4chart = st.dataframe(df.iloc[:,[0,1,2,13,14,15,16,17]].sort_values(['년도','나이대'],axis=0))
            else:
                df_4chart = st.dataframe(df.iloc[:,[0,1,2,18,19,20,21,22]].sort_values(['년도','나이대'],axis=0))
